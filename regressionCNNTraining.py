...
def simulate_feed_variance(N_simulation_steps, N, splotches, delta_t, DA, DB, kill, feedLower, feedUpper, increment):
    A, B = official_initial_config(N, splotches)
    for j in range(int((feedUpper - feedLower) / increment)):

        for t in range(N_simulation_steps):
            A, B = gray_scott_update(A, B, DA, DB, feedLower, k, delta_t)

        print("A:", A, "B:", B)
        print(torch.std(A), torch.std(B))
        if (torch.std(A) > 0.00001) and (torch.std(B) > 0.00001):
            reactionParameters = [DA, DB, feedLower, kill]
            np.savez("Equilibria" + str(j) + ".npz", EquilibriaA=A,
                     EquilibriaB=B, parameters=reactionParameters)
...
def newLoss(bX, pred, bY, batchSize):
    ...
    #print(abs(predA - truthA), abs(predB - truthB) )
...
#Function to load Equilibria from .npz files
...
EquilibriaTensors, TruthLabels = loadEquilibriumFilesForRegressionTask("/Users/zachzerbe/PycharmProjects/SpatialPatternPredictionResearch")


# ** NOTE that in this version I have hard coded our new loss function into the train and test loops
